...
from services import logging_db
...
def test_log_transaction_change():
    ...
    class FakeCursor:
        def execute(self, query, params=None): executed["call"] = (query.strip().lower(), params)
    ...
    with patch("services.logging_db.get_cursor", return_value=FakeCursor()):
        logging_db.log_transaction_change(
            transaction_id=42,
            action="update",
...
def test_log_title_change_inserts(monkeypatch):
    ...
    logging_db.log_title_change("test@example.com", "CB", "admin@example.com")
...
def test_log_residency_change_inserts(monkeypatch):
    ...
    monkeypatch.setattr("services.logging_db.get_cursor", lambda: FakeCursor())
    logging_db.log_residency_change("test@example.com", True, "admin@example.com")
